...
<!-- **************************************** -->
<center>
<h1>DEfO ECH Smokeping-like Check for pyhon</h1>

<p>The (server-side) script on this page displays the results from attempting
ECH (via a pyhon script) with each of the URLs shown, for the time of the run shown.
...
<?php
    ...
    $topdir="/var/extra/smokeping/python-runs";
    ...
    if ($runind>0) {
    	...
    	//var_dump($prevtime);
    	$prevurl="https://test.defo.ie/smokeping-python.php?runtime=".$prevtime;
    ...
    if ($thisrun!=$last) {
    	...
    	//var_dump($nexttime);
    	$nexturl="https://test.defo.ie/smokeping-python.php?runtime=".$nexttime;
    ...
    $verinfo=file_get_contents($thisrun."/".$runtime.".python.ver");
