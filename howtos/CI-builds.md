...
The goal of our continuous integration (CI) builds and tests is to identify
...
For curl, we no longer merge the curl code from the DEfO-project
repo, rather using the debian sources as those now also contain
our ECH code. We do continue to use the repo for the CI build
and test though.
...
- attempt a `git merge` with the upstream for the relevant project
  (other than with curl)
- build the relevant ECH-enabled debian package resulting from the merge
- store built debian packages in a "pseudo" branch called `packages`
- attempt a basic test using the test scripts from this repo and those packages

When running those as part of this CI setup, we set
an environment variable `PACKAGING=1` so the scripts and configs
can differ when run from the command line and CI environments, which is
sometimes useful.
...
```bash
$ cd $HOME/lt
$ ATOP=$HOME/code/defo-project-org/apache-httpd-rebase/ $HOME/code/defo-project-org/ech-dev-utils/scripts/testapache.sh
/home/user/lt
Executing: /home/user/code/defo-project-org/apache-httpd-rebase//httpd -d /home/user/lt -f /home/user/code/defo-project-orgech-dev-utils/configs/apachemin.conf
Testing grease 9443
Testing public 9443
Testing real 9443
Testing hrr 9443
